<template>
  <div>
    ...
    <ul class="page-list">
      <li class="page-item" v-for="(item, index) in markList" @click="getIn(item)" v-clickdown :key="index">
    ...
    <ul class="page-list">
      <li class="page-item" v-for="(item, index) in friendList" :key="index">
        <img class="page-header" :src="item.header" alr="头像加载失败" />
        <div class="page-text-box">
          <div class="page-text contact">
            <p class="page-text-head">
...
<script>
import {FriendList} from "./contact"
export default {
    data () {
        return {
            pageName: "通讯录",
